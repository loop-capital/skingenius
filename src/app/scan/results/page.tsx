  ...
  const callApi = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/v1/scan", {
        ...
        body: JSON.stringify({
          image: imageData,
          capture_method: state.uploadedImageUri ? "gallery" : "camera",
          skin_tone: skinTone,
        }),
      });

      const json: V1ScanResponse = await res.json();
      ...
      if (json.data) {
        ...
        dispatch({
          type: "SET_RESULTS",
          analysisId: json.data.scan_id ?? "",
          conditions:
            json.data.conditions?.map((c) => ({
              name: c.name,
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-[100dvh] px-6 pt-8 pb-8 items-center justify-center text-center">
        ...
        <p className="text-stone-600 font-medium">Loading results...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-[100dvh]">
      ...
      <main className="flex-1 px-6 pb-8 space-y-6">
          ...
          <Card className="border-stone-200">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${qa.is_valid_format ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${qa.face_detected ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}>
              ...
              {qa.blur_score != null && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-stone-900">{qa.blur_score.toFixed(1)}</span>
                </div>
              )}
              {qa.lighting_score != null && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-stone-600">Lighting Score</span>
                  <span className="text-sm font-medium text-stone-900">{qa.lighting_score.toFixed(1)}</span>
                </div>
              )}
              {data.metadata?.warnings && data.metadata.warnings.length > 0 && (
                <div className="mt-2 pt-3 border-t border-stone-100">
                  <p className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-1">Warnings</p>
                ...
                <Card key={`${z.zone}-${i}`} className="border-stone-200">
                  <CardContent className="p-4">
                    ...
                    <div className="mt-2 flex items-center gap-1">
                      ...
                      <span className="text-xs font-medium text-emerald-700">{confidencePercent(z.confidence)}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        <Alert className="bg-amber-50 border-amber-200">
          ...
          <AlertTitle className="text-amber-800 text-sm">Medical Disclaimer</AlertTitle>
          <AlertDescription className="text-amber-700 text-xs">
            This analysis is for informational purposes only and does not constitute a medical diagnosis.
            If you have concerns about your skin health, please consult a board-certified dermatologist.
          </AlertDescription>
        </Alert>
      </main>
