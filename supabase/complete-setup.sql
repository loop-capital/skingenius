...
-- ============================================
-- INGREDIENTS TABLE (must be first - other tables may reference it)
-- ============================================

-- First, drop the old constraint if it exists
ALTER TABLE IF EXISTS public.ingredients DROP CONSTRAINT IF EXISTS ingredients_category_check;
ALTER TABLE IF EXISTS public.ingredients DROP CONSTRAINT IF EXISTS ingredients_evidence_level_check;

-- Now add the comprehensive category constraint
ALTER TABLE IF EXISTS public.ingredients ADD CONSTRAINT ingredients_category_check 
  CHECK (category IN (
...
ALTER TABLE IF EXISTS public.ingredients ADD CONSTRAINT ingredients_evidence_level_check 
  CHECK (evidence_level IN ('strong', 'moderate', 'emerging', 'limited', 'A', 'A-', 'B', 'B+', 'B-', 'C', 'C+', 'C-', 'D'));

-- Also ensure ingredients table exists with proper structure
CREATE TABLE IF NOT EXISTS public.ingredients (
  ...
  min_concentration DECIMAL(8,2),
  max_concentration DECIMAL(8,2),
