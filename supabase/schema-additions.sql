...
CREATE TABLE IF NOT EXISTS public.pro_subscriptions (
  ...
  billing_provider TEXT DEFAULT 'square', -- square or stripe
  billing_subscription_id TEXT,
  monthly_price_cents INTEGER NOT NULL DEFAULT 2900, -- $29/mo default
  max_clients INTEGER DEFAULT 50, -- pro=50, enterprise=unlimited
...
-- Client-Pro relationships (user opts in to share with a pro)
CREATE TABLE IF NOT EXISTS public.client_pro_relationships (
  ...
  invited_by TEXT CHECK (invited_by IN ('client', 'pro')), -- who initiated
  invite_code TEXT, -- for pro-generated invite links
  ...
  pro_can_message BOOLEAN DEFAULT TRUE, -- pro can send check-in prompts
...
-- ============================================
-- SCAN RESULTS (v1 pipeline)
-- ============================================

CREATE TABLE IF NOT EXISTS public.scan_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  photo_id UUID REFERENCES public.skin_photos(id) ON DELETE SET NULL,
  capture_method TEXT NOT NULL CHECK (capture_method IN ('camera', 'gallery')),
  skin_tone INTEGER NOT NULL CHECK (skin_tone BETWEEN 1 AND 6),
  quality_assessment JSONB NOT NULL DEFAULT '{}',
  conditions JSONB NOT NULL DEFAULT '[]',
  skin_zones JSONB NOT NULL DEFAULT '[]',
  metadata JSONB NOT NULL DEFAULT '{}',
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_scan_results_user ON public.scan_results(user_id);
CREATE INDEX IF NOT EXISTS idx_scan_results_created ON public.scan_results(created_at);
...
-- Pro check-in messages sent to clients
CREATE TABLE IF NOT EXISTS public.pro_messages (
  ...
  body TEXT NOT NULL,
  includes_scan_request BOOLEAN DEFAULT FALSE, -- pro asks client to do a new scan
  scan_id UUID REFERENCES public.scan_results(id), -- if linked to a specific scan
  read_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Referral queue: consumers matched to pros
CREATE TABLE IF NOT EXISTS public.referral_queue (
  ...
  preferred_service_types TEXT[] DEFAULT '{}', -- e.g., ['facial', 'chemical-peel', 'microneedling']
  matched_pro_id UUID REFERENCES public.profiles(id),
  match_score FLOAT, -- how well the pro matches the client's needs
